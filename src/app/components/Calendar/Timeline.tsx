...
const Timeline = ({ height, pxPerMin, timeLineStart, timeLineEnd }: Props) => {
  ...
  const elements = useMemo(() => {
    ...
    for (let i = timeLineStart; i <= timeLineEnd; i++) {
      elems.push(
        <div key={i} className={`border-gray-400 border-t-2 border-dotted relative`} style={{ height: `${height}px` }}>
  ...
  const [currTimeMins, setCurrTimeMins] = useState<number>(res[0]);
  const [position, setPosition] = useState<number>(res[1]);

  useEffect(() => {
    const updateCurrTime = () => {
      const [mins, pos] = calculateCurrTime(pxPerMin, timeLineStart,timeLineEnd);
  ...
  return (
    <div className="relative">
      {elements}
      <div className="border-t-2 border-blue-600 absolute w-full z-10" ref={timeRef} style={{ top: `${position}px` }}>
